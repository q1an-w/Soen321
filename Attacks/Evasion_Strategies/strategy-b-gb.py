...
# Load data generated from surrogate Gradient Boosting model
...
X_scaled = scaler.transform(X_mod.values)  # using .values to bypass feature name mismatch

# Predict and evaluate
predictions = model.predict(X_scaled)
frauds["Prediction"] = predictions
frauds["Evasion_Success"] = predictions == 0
frauds.to_csv("results_strategy_b_gb.csv", index=False)

success = np.sum(predictions == 0)
total = len(predictions)
print(f"[GB] Strategy B: {success}/{total} successful evasions ({(success/total)*100:.2f}%)")
